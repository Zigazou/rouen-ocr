...
class HtmlCorrections(HtmlWork):
    ...
    def group_captions_with_images(self) -> Self:
        """Group captions with their corresponding images.

        This method groups captions with their corresponding images. The
        captions generated by the OCR are identified by a div with a
        ``data-label="Caption"`` attribute.
        
        The method looks for the nearest ``img`` tag in the same ``section``. It
        embeds the ``img`` inside a ``figure`` tag. It moves the caption in a
        ``figcaption`` tag inside this same ``figure``.
        """
        self.must_not_have("remove_chandra_divs")

        imgs = self.soup.find_all("img")
        for img in imgs:
            # Get the div containing the img.
            if not img.parent or not img.parent.parent:
                continue

            image_div = img.parent

            # Get the caption div that is a sibling of the image div.
            caption = image_div.find_next_sibling(
                "div",
                attrs={"data-label": "Caption"}
            )

            if (
                not caption or
                not isinstance(caption, Tag) or
                caption.name != "div" or
                caption.get("data-label") != "Caption"
            ) :
                continue

            # Move the img tag inside a figure tag.
            figure = self.soup.new_tag("figure")
            img.insert_before(figure)
            figure.append(img.extract())
            
            # Move the caption inside a figcaption inside the figure tag.
            figcaption_tag = self.soup.new_tag("figcaption")
            figcaption_tag.string = caption.get_text(strip=True)
            figure.append(figcaption_tag)

            # Remove the original caption div.
            caption.decompose()

        self.remember("group_captions_with_images")
